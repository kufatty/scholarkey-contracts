...
export default function ViewInformation(){
  ...
  async function onMyRole(){
    try {
      ...
      setOutput(`Your role: ${roleNames[roleIndex] || 'Unknown'} (${roleIndex})`)
    } catch (e: any){
  ...
  async function onViewGradeById(id: string){
    try {
      ...
      const [student, courseCode, grade, semester, status] = await contract.getGradeInfo(gradeId)
      setOutput(
  ...
  async function onViewMyGrades(){
    try {
      ...
      const lines = grades.map((g: any, i: number) =>
        `${i + 1}. ${g.courseCode} - ${g.grade.toString()}/20 (ID ${g.id.toString()}, ${g.semester}, ${g.status})`
      )
    ...
    <div style={{marginTop:24}}>
      ...
      <div style={{display:'grid', gap:8}}>
        ...
        <div>
          ...
          <input value={gradeIdInput} onChange={e => setGradeIdInput(e.target.value)} placeholder="Grade ID" />
          <button onClick={() => onViewGradeById(gradeIdInput)}>View grade</button>
        </div>
        ...
        <div>
          ...
          <input value={studentForView} onChange={e => setStudentForView(e.target.value)} placeholder="Student address" />
          <button onClick={() => onViewStudentGrades(studentForView)}>View student grades</button>
        </div>
