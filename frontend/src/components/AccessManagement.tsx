...
export default function AccessManagement() {
  ...
  async function onCheckAccess() {
    try {
      ...
      const contract = await getContract();
      if (!contract) throw new Error("Contract not configured");
      const hasAccess: boolean = await contract.hasAccess(
        studentToCheck,
        viewer
      );
      if (hasAccess) {
        const count: bigint = await contract.getStudentGradeCount(
          studentToCheck
        );
  ...
  return (
    <div style={{ marginTop: 24 }}>
      ...
      <div style={{ display: "grid", gap: 8 }}>
        ...
        <button onClick={onViewGrantedList}>
          View who has access to my grades
        </button>

        <label>Check if I have access to a student</label>
        <input
          value={studentToCheck}
          onChange={(e) => setStudentToCheck(e.target.value)}
