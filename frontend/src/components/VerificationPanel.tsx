...
export default function VerificationPanel() {
  ...
  async function onVerifyGrade() {
    try {
      ...
      const signature = await generateSignatureForExistingGrade(
        contract,
        signer,
        id,
        "verify"
  ...
  async function onVerifySignatures() {
    try {
      ...
      const [teacherValid, deptValid, directorValid, allValid] =
        await contract.verifyGradeSignatures(id);
      const lines = [
  ...
  async function onCheckFinalized() {
    try {
      ...
      let text = finalized
        ? "Grade is FINALIZED and immutable"
        : "Grade is NOT finalized";
      try {
  ...
  async function onVerifyStudentGrade() {
    try {
      if (!studentForCourse || !courseForVerify)
        throw new Error("Student and course required");
      const contract = await getContract();
      if (!contract) throw new Error("Contract not configured");
      const [exists, gradeValue, status] = await contract.verifyStudentGrade(
        studentForCourse,
        courseForVerify
      );
      if (!exists) {
  ...
  return (
    <div style={{ marginTop: 24 }}>
      ...
      <div style={{ display: "grid", gap: 8 }}>
        ...
        <input
          value={studentForCourse}
          onChange={(e) => setStudentForCourse(e.target.value)}
          placeholder="Student address"
        />
        <input
          value={courseForVerify}
          onChange={(e) => setCourseForVerify(e.target.value)}
